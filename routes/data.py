...
async def upload_data(request : Request, project_id : str, file : UploadFile,
    ...
    # add the file to the assets collection 
    ...
    return JSONResponse(
            content={
                ...
                "file ID" : file_id, 
                "asset_rec" : str(asset_record.asset_pushed_at)
...
async def process_file(request :Request , project_id: str , process_request: ProcessRequest): 
    ...
    record = await asset_model.get_asset_by_name(
        asset_project_id = project.id,
        asset_name= file_id
    )
    if record is None : 
    ...
    file_asset_id , file_id = record.id, record.asset_name 
...
@data_router.post('/process_all_project_files/{project_id}')
async def process_all_files_with_type(request: Request, project_id: str, process_request: ProcessRequest): 
    # no file is we will process all files 
    chunk_size = process_request.chunk_size
    overlap_size = process_request.overlap_size
    do_reset = process_request.do_reset
    ...
    asset_model = await AssetModel.create_instance(db_client=request.app.db_client)
    all_project_files_records = await asset_model.get_all_project_assets(
        asset_project_id = project.id,
        asset_type=AssetTypeEnum.FILE.value
    )
    if not all_project_files_records: 
        return JSONResponse(
    ...
    all_files_ids = {
        ...
        for rec in all_project_files_records
    }
    ...
    files_processed, unprocessed_files = 0, 0
    file_chunks_recordes = []

    for file_asset_id , file_id in all_files_ids.items(): 
